...
// Mail domain entities (queue, contractor mail, audits, attachments, simple mail)
...
const DEMO_SEED = {
  projectCount: Number.parseInt(process.env.DEMO_PROJECT_COUNT ?? '') || 13,          // at least a dozen
...
export async function seedInitialData() {
    ...
    const subRolePermSeeds = [
        ...
        // Documents (skipped seeding of docs, but keep perms)
        ...
        // Impediments (kept in perms but NOT seeded as data per request)
    ...
    // Curated projects including Finance/Support domains
    type ProjectSeed = {
        title: string;
        description: string;
    ...
    // Build project teams, tasks (including epics/subtasks), logs, time entries, dependencies
    ...
    for (let pi = 0; pi < seededProjects.length; pi++) {
        ...
        // Assemble a team: DEMO_TEAM_MIN..DEMO_TEAM_MAX users including a lead
        const team: User[] = [];
        ...
        // Target total tasks for this project
        ...
        // Additional flat tasks to reach taskTarget
        ...
        // Dependencies among tasks (chain + cross-links)
    ...
    // MAIL SEEDING (no impediments/documents seeded; mails included)
